...
# Define the command
command="geth --identity 'node4' --syncmode 'full' --ws --ws.addr $IP_NODE_4  --ws.port $WS_PORT_NODE_4 --datadir node4 --port $ETH_PORT_NODE_4 --bootnodes $BOOTNODE_URL --ws.api 'eth,net,web3,personal,miner,admin,clique' --networkid $NETWORK_ID --nat 'any' --allow-insecure-unlock --authrpc.port $RPC_PORT_NODE_4 --ipcdisable --unlock $ETHERBASE_NODE_4 --password password.txt --mine --snapshot=false --miner.gaslimit 8000000 --miner.etherbase $ETHERBASE_NODE_4 --ethstats node4:$WS_SECRET@$ETH_NETSATS_IP:$ETH_NETSATS_PORT" 
